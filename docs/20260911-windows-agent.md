...
2. 首次配置 Windows 可访问的 WSS 地址，例如 `wss://proxy.example.com/agent`，并选择新主机的默认客户端凭据。
3. 点击 **生成 Token**，复制 Token 或完整启动命令。多台 Windows 共用此密钥，无需逐台申请或预建主机。
4. Token 在服务设置中可再次查看和复制；完整 Token 加密保存在数据库中，认证使用 SHA-256 摘要。数据库必须连同 `.db.key` 一起备份。

仅传 `-token` 即可连接，因为 Token 包含服务器地址和随机密钥。后台 HTTP 地址不等于可用的 WSS 地址，需要先配置 HTTPS。修改连接地址和默认凭据后，通过 **重新生成 Token** 应用；已有主机访问权限不会被覆盖。
...
- **重新生成 Token** 或 **停用自注册** 会立即断开使用该密钥的 Agent，旧 Token 无法注册或重连。轮换后用新 Token 和原名称启动，继续使用原主机记录与访问权限。
- 默认客户端凭据只在首次注册时复制到新主机，后续在服务器/客户端凭据页面维护。
...
以下示例在本地 PowerShell 中执行，SSH 登录凭据沿用已有配置；将 `windows-es` 换成自动注册后显示的主机名或 SSH 登录名：
...
- 修改服务器配置、禁用、删除或重新生成设备凭据会断开对应 Agent。旧 SSH 连接每次发起新任务时仍检查设备是否可用。
...
- 审计会记录命令和输出，不能自动识别所有密码。避免输出凭据文件、完整环境变量和含密码的配置；ES 认证应在目标机器本地处理。
...
- 已验证独立设备凭据认证、凭据轮换失效、SSH 断线取消、忙时拒绝和审计输出。
...
- 已核对：35 台主机、3 份服务器凭据、3 份客户端凭据、36 条绑定、1 个管理员、1624 条历史审计全部保留。除服务器凭据由明文转为加密存储外，旧字段内容逐项核对一致；审计按原始字节比对，保留历史非 UTF-8 内容。
- 新建的 `claude-ssh-proxy.db.key` 是现有凭据的加密密钥，后续必须与数据库一起备份。管理员密码、JWT 设置和 SSH host key 均保留。
- 已验证新后台资源、未认证接口拒绝访问、SSH 握手、SQLite 完整性和外键，以及正式数据库副本的凭据解密。未逐台测试 35 台后端主机的实时连通性。
...
- 本机测试 Agent、服务器临时监听均已停止，专用测试设备和客户端凭据已移除，测试配置已失效。原有主机未修改。
...
- 网页新增“Agent 接入”：先选择 WSS 地址和允许访问的客户端凭据，再申请一次显示的接入 Token。
...
- 正式库升级后逐项核对完全一致：39 台主机、3 份服务器凭据、3 份客户端凭据、39 条绑定、1 个管理员、1 条设置和 1651 条审计。SQLite 完整性和外键检查通过，加密密钥和 SSH host key 未变。
...
- 如同时更名 systemd 服务，先停用旧服务，再创建 `ops-ssh-proxy.service`，用新程序路径和原有数据路径/监听参数启动。不要让两个服务同时访问同一数据库。成功后核对主机、凭据、审计及密钥；失败时停新服务，成套恢复备份并启用旧服务。
