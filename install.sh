...
readonly SERVICE_NAME="aiagent-ssh-proxy"
readonly INSTALL_DIR="${OPS_SSH_PROXY_INSTALL_DIR:-/data/aiagent-ssh-proxy}"
readonly LEGACY_DATA_DIR="${OPS_SSH_PROXY_LEGACY_DATA_DIR:-/var/lib/aiagent-ssh-proxy}"
readonly UNIT_PATH="${OPS_SSH_PROXY_UNIT_PATH:-/etc/systemd/system/${SERVICE_NAME}.service}"
readonly SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
readonly SOURCE_BINARY="${SCRIPT_DIR}/aiagent-ssh-proxy"
...
for old_service in ops-ssh-proxy claude-ssh-proxy; do
  if systemctl cat "${old_service}.service" >/dev/null 2>&1 || [[ -f /data/${old_service}/${old_service}.db || -f /var/lib/${old_service}/${old_service}.db ]]; then
    echo "检测到 ${old_service} 旧部署。请按 Windows Agent 文档中的更名迁移说明保留数据和监听参数后迁移，不可作为全新安装覆盖。" >&2
    exit 1
  fi
done
...
if [[ ! -e ${INSTALL_DIR}/aiagent-ssh-proxy.db && -f ${LEGACY_DATA_DIR}/aiagent-ssh-proxy.db ]]; then
  echo "检测到旧数据目录,正在复制数据到 ${INSTALL_DIR}..."
  for name in aiagent-ssh-proxy.db aiagent-ssh-proxy.db-wal aiagent-ssh-proxy.db-shm aiagent-ssh-proxy.db.key host_key; do
...
if [[ -f ${INSTALL_DIR}/aiagent-ssh-proxy.db ]]; then
